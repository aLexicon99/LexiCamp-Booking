...
export default function BookingDetails({ campground, checkin, checkout }) {
  ...
  const cleaningFee = Math.ceil(
    campground.pricePerNight * CLEANING_FEE_PERCENTAGE,
  );
  ...
  return (
    <div className="bg-white rounded-xl shadow-[0_8px_30px_rgba(27,67,50,0.08)] border border-stone-100 overflow-hidden">
      <div className="relative h-48 w-full">
        ...
        />
        <div className="absolute top-4 right-4 bg-white/90 backdrop-blur-md px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-tighter text-emerald-900">
          Rare Find
        </div>
      </div>
      <div className="p-8">
        <div className="mb-6">
          ...
          <p className="text-stone-500 text-sm mt-1">{campground.city}</p>
        </div>
        ...
        <div className="space-y-4 mb-8">
          <div className="flex justify-between text-stone-600">
            <span>
              {campground.pricePerNight} kr x {nights}{" "}
              {nights < 2 ? "night" : "nights"}
            </span>
